...
import type RouteValidator from "./RouteValidator.js";

export default class ZaladoAPIGuideBasedRouteValidator
  implements RouteValidator
{
  private readonly ZALANDO_API_GUIDE_BASED_ROUTE_REGEX = new RegExp(
    "^/(?:(?:[a-z][a-z0-9-]*|{[A-Za-z0-9_]+}))(?:/(?:[a-z][a-z0-9-]*|{[A-Za-z0-9_]+}))*$",
  );
  ...
  public validate(route: Route): void {
    ...
    if (!this.isValid(route.value)) {
      throw new Error(
        "Invalid Route Pattern. Routes must have the following pattern: /path or /path/{param}, kebab-case segments only.",
      );
    }
  }
